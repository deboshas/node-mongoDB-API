...
MongoClient.connect('mongodb://localhost:27017/ToDoApp', (error, db) => {
    ...
    //db.collection('Users').deleteMany({ Name: "Debojyoti" }).then((docs) => {
    ...
    db.collection('Users').deleteOne({ Name: "Rai" }).then((result) => {
        console.log(JSON.stringify(result, undefined, 2));
    })
    db.collection('Users').findOneAndDelete({ Name: "ChdirVai" }).then((result) => {
        console.log(JSON.stringify(result, undefined, 2));
    })
